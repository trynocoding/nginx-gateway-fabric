---
title: "examples/tcp-routing/tcp-routes.yaml 源码分析"
tags:
...
sources:
  - repo: nginx-gateway-fabric
    ...
    dirty: false
source_file: examples/tcp-routing/tcp-routes.yaml
source_sha256_12: 01067fc6dc09
runtime_snapshots: []
---

# examples/tcp-routing/tcp-routes.yaml 源码分析

> [!abstract] 核心结论
> 该文件声明 2 个对象（TCPRoute）。它的核心价值不是展示 YAML 语法，而是提供一段可追踪输入：受管对象经过 Graph 验证与引用解析，普通工作负载则只通过 Service/Secret 等边界间接影响 NGF。

## 为什么选它

此文件属于二八抽样中的代表输入：它覆盖 **TCPRoute** 的关键处理面，并能与同目录文件组成完整或近完整的因果链。重复的 Deployment/Service 变体、只改变名称的 Route 和操作型 README 不再逐篇展开。

## 文件现场与职责边界

| 文档 | API 版本 | Kind | Namespace | Name |
|---:|---|---|---|---|
| 1 | `gateway.networking.k8s.io/v1alpha2` | `TCPRoute` | `default/继承 kubectl 上下文` | `tcp-coffee` |
| 2 | `gateway.networking.k8s.io/v1alpha2` | `TCPRoute` | `default/继承 kubectl 上下文` | `tcp-tea` |

**源码事实**：文件指纹为 `01067fc6dc09`，分析基于 revision `87e0580143fc`。本文没有把案例实际部署到集群，因此“生成了某条 NGINX 指令”属于源码可推导的预期，不是运行观察。
...
| 对象 | 字段 | 案例值 | 进入系统后的含义 |
|---:|---|---|---|
| 1 | `metadata.name` | `tcp-coffee` | 组成 NamespacedName，是 Graph、引用解析与生成文件命名的稳定身份。 |
| 1 | `spec.parentRefs[0].name` | `gateway` | Route→listener 附着入口；group/kind/name/sectionName/namespace 决定候选父对象。 |
| 1 | `spec.parentRefs[0].sectionName` | `coffee` | Route→listener 附着入口；group/kind/name/sectionName/namespace 决定候选父对象。 |
| 1 | `spec.rules[0].backendRefs[0].name` | `coffee` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
| 1 | `spec.rules[0].backendRefs[0].port` | `8081` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
| 2 | `metadata.name` | `tcp-tea` | 组成 NamespacedName，是 Graph、引用解析与生成文件命名的稳定身份。 |
| 2 | `spec.parentRefs[0].name` | `gateway` | Route→listener 附着入口；group/kind/name/sectionName/namespace 决定候选父对象。 |
| 2 | `spec.parentRefs[0].sectionName` | `tea` | Route→listener 附着入口；group/kind/name/sectionName/namespace 决定候选父对象。 |
| 2 | `spec.rules[0].backendRefs[0].name` | `tea` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
| 2 | `spec.rules[0].backendRefs[0].port` | `8082` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
...
|---|---|---|---|
| `TCPRoute` | `internal/controller/state/graph/tcproute.go:buildTCPRoute` | 把监听器与 Service backend 转成 stream TCP server/upstream | `internal/controller/state/graph/tcproute_test.go` |
...
1. 静态校验：`kubectl apply --dry-run=server -f examples/tcp-routing/tcp-routes.yaml`。
...
4. 回归测试入口：`internal/controller/state/graph/tcproute_test.go`。
...
- [[tcp-routing/gateway-yaml|gateway.yaml]]
- [[00-首页-学习路线]]
- [[99-源码索引与术语表]]
