---
title: "examples/tcp-routing/gateway.yaml 源码分析"
tags:
...
sources:
  - repo: nginx-gateway-fabric
    ...
    dirty: false
source_file: examples/tcp-routing/gateway.yaml
source_sha256_12: b0f5173a6392
runtime_snapshots: []
---

# examples/tcp-routing/gateway.yaml 源码分析
...
**源码事实**：文件指纹为 `b0f5173a6392`，分析基于 revision `87e0580143fc`。本文没有把案例实际部署到集群，因此“生成了某条 NGINX 指令”属于源码可推导的预期，不是运行观察。
...
| 1 | `spec.listeners[0].name` | `coffee` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[0].protocol` | `TCP` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[0].port` | `81` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[0].allowedRoutes.kinds[0]` | `{...}` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].name` | `tea` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].protocol` | `TCP` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].port` | `82` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].allowedRoutes.kinds[0]` | `{...}` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
...
1. 静态校验：`kubectl apply --dry-run=server -f examples/tcp-routing/gateway.yaml`。
...
- [[tcp-routing/tcp-routes-yaml|tcp-routes.yaml]]
- [[00-首页-学习路线]]
- [[99-源码索引与术语表]]
