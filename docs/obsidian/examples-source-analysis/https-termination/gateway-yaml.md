---
title: "examples/https-termination/gateway.yaml 源码分析"
tags:
...
sources:
  - repo: nginx-gateway-fabric
    ...
    dirty: false
source_file: examples/https-termination/gateway.yaml
source_sha256_12: 133183293194
runtime_snapshots: []
---

# examples/https-termination/gateway.yaml 源码分析

> [!abstract] 核心结论
> 该文件声明 1 个对象（Gateway）。它的核心价值不是展示 YAML 语法，而是提供一段可追踪输入：受管对象经过 Graph 验证与引用解析，普通工作负载则只通过 Service/Secret 等边界间接影响 NGF。

## 为什么选它

此文件属于二八抽样中的代表输入：它覆盖 **Gateway** 的关键处理面，并能与同目录文件组成完整或近完整的因果链。重复的 Deployment/Service 变体、只改变名称的 Route 和操作型 README 不再逐篇展开。

## 文件现场与职责边界

| 文档 | API 版本 | Kind | Namespace | Name |
|---:|---|---|---|---|
| 1 | `gateway.networking.k8s.io/v1` | `Gateway` | `default/继承 kubectl 上下文` | `gateway` |

**源码事实**：文件指纹为 `133183293194`，分析基于 revision `87e0580143fc`。本文没有把案例实际部署到集群，因此“生成了某条 NGINX 指令”属于源码可推导的预期，不是运行观察。
...
| 对象 | 字段 | 案例值 | 进入系统后的含义 |
|---:|---|---|---|
| 1 | `metadata.name` | `gateway` | 组成 NamespacedName，是 Graph、引用解析与生成文件命名的稳定身份。 |
...
| 1 | `spec.listeners[1].name` | `https` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].port` | `443` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].protocol` | `HTTPS` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].tls.mode` | `Terminate` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].tls.certificateRefs[0]` | `{...}` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].tls.options.nginx.org/ssl-protocols` | `TLSv1.2 TLSv1.3` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
| 1 | `spec.listeners[1].tls.options.nginx.org/ssl-ciphers` | `ECDHE-RSA-AES128-GCM-SHA256:ECDHE-RSA-AES256-GCM-SHA384:HIGH:!aNULL:!MD5` | 声明监听端口、协议、hostname、TLS 与允许附着的 Route 范围。 |
...
1. 静态校验：`kubectl apply --dry-run=server -f examples/https-termination/gateway.yaml`。
...
4. 回归测试入口：`internal/controller/state/graph/gateway_test.go`。
