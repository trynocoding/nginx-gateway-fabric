---
title: "examples/snippets/limit-except-sf.yaml 源码分析"
tags:
...
sources:
  - repo: nginx-gateway-fabric
    ...
    dirty: false
source_file: examples/snippets/limit-except-sf.yaml
source_sha256_12: 83176bfb8216
runtime_snapshots: []
---

# examples/snippets/limit-except-sf.yaml 源码分析

> [!abstract] 核心结论
> 该文件声明 1 个对象（SnippetsFilter）。它的核心价值不是展示 YAML 语法，而是提供一段可追踪输入：受管对象经过 Graph 验证与引用解析，普通工作负载则只通过 Service/Secret 等边界间接影响 NGF。

## 为什么选它

此文件属于二八抽样中的代表输入：它覆盖 **SnippetsFilter** 的关键处理面，并能与同目录文件组成完整或近完整的因果链。重复的 Deployment/Service 变体、只改变名称的 Route 和操作型 README 不再逐篇展开。

## 文件现场与职责边界

| 文档 | API 版本 | Kind | Namespace | Name |
|---:|---|---|---|---|
| 1 | `gateway.nginx.org/v1alpha1` | `SnippetsFilter` | `default/继承 kubectl 上下文` | `limit-except-sf` |

**源码事实**：文件指纹为 `83176bfb8216`，分析基于 revision `87e0580143fc`。本文没有把案例实际部署到集群，因此“生成了某条 NGINX 指令”属于源码可推导的预期，不是运行观察。
...
| 对象 | 字段 | 案例值 | 进入系统后的含义 |
|---:|---|---|---|
| 1 | `metadata.name` | `limit-except-sf` | 组成 NamespacedName，是 Graph、引用解析与生成文件命名的稳定身份。 |
| 1 | `spec.snippets[0].context` | `http.server.location` | 原样 NGINX 指令是高权限扩展面，需功能开关并受允许上下文限制。 |
| 1 | `spec.snippets[0].value` | `limit_except GET {\n  deny all;\n}\n` | 原样 NGINX 指令是高权限扩展面，需功能开关并受允许上下文限制。 |
...
|---|---|---|---|
| `SnippetsFilter` | `internal/controller/state/graph/snippets_filter.go:processSnippetsFilters` | 由 HTTPRoute ExtensionRef 引用，把 server/location 片段转成 include；依赖显式功能开关 | `internal/controller/state/graph/snippets_filter_test.go` |
...
1. 静态校验：`kubectl apply --dry-run=server -f examples/snippets/limit-except-sf.yaml`。
...
4. 回归测试入口：`internal/controller/state/graph/snippets_filter_test.go`。
...
- [[snippets/limit-conn-sp-yaml|limit-conn-sp.yaml]]
- [[00-首页-学习路线]]
- [[99-源码索引与术语表]]
