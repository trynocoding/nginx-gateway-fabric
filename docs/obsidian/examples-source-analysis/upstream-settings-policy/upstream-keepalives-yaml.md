---
title: "examples/upstream-settings-policy/upstream-keepalives.yaml 源码分析"
tags:
...
sources:
  - repo: nginx-gateway-fabric
    ...
    dirty: false
source_file: examples/upstream-settings-policy/upstream-keepalives.yaml
source_sha256_12: 0eb84b0918f1
runtime_snapshots: []
---

# examples/upstream-settings-policy/upstream-keepalives.yaml 源码分析

> [!abstract] 核心结论
> 该文件声明 1 个对象（UpstreamSettingsPolicy）。它的核心价值不是展示 YAML 语法，而是提供一段可追踪输入：受管对象经过 Graph 验证与引用解析，普通工作负载则只通过 Service/Secret 等边界间接影响 NGF。

## 为什么选它

此文件属于二八抽样中的代表输入：它覆盖 **UpstreamSettingsPolicy** 的关键处理面，并能与同目录文件组成完整或近完整的因果链。重复的 Deployment/Service 变体、只改变名称的 Route 和操作型 README 不再逐篇展开。

## 文件现场与职责边界

| 文档 | API 版本 | Kind | Namespace | Name |
|---:|---|---|---|---|
| 1 | `gateway.nginx.org/v1alpha1` | `UpstreamSettingsPolicy` | `default/继承 kubectl 上下文` | `upstream-keepalives` |

**源码事实**：文件指纹为 `0eb84b0918f1`，分析基于 revision `87e0580143fc`。本文没有把案例实际部署到集群，因此“生成了某条 NGINX 指令”属于源码可推导的预期，不是运行观察。
...
| 对象 | 字段 | 案例值 | 进入系统后的含义 |
|---:|---|---|---|
| 1 | `metadata.name` | `upstream-keepalives` | 组成 NamespacedName，是 Graph、引用解析与生成文件命名的稳定身份。 |
| 1 | `spec.targetRefs[0].group` | `core` | Policy attachment 入口；目标类型与作用域决定策略进入 server、location 或 upstream。 |
| 1 | `spec.targetRefs[0].kind` | `Service` | Policy attachment 入口；目标类型与作用域决定策略进入 server、location 或 upstream。 |
| 1 | `spec.targetRefs[0].name` | `coffee` | Policy attachment 入口；目标类型与作用域决定策略进入 server、location 或 upstream。 |
| 1 | `spec.keepAlive.connections` | `32` | 由该资源所属 API/外部控制器消费；是否影响 NGF 取决于它是否处于 Gateway 可达引用链。 |
...
|---|---|---|---|
| `UpstreamSettingsPolicy` | `internal/controller/state/graph/policies.go:processPolicies` | 以 Service 为目标，影响 upstream zone、keepalive、负载均衡等后端组配置 | `internal/controller/nginx/config/policies/upstreamsettings/validator_test.go` 与 `processor_test.go` |
...
1. 静态校验：`kubectl apply --dry-run=server -f examples/upstream-settings-policy/upstream-keepalives.yaml`。
...
4. 回归测试入口：`internal/controller/nginx/config/policies/upstreamsettings/validator_test.go` 与 `processor_test.go`。
