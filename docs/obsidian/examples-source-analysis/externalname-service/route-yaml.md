---
title: "examples/externalname-service/route.yaml 源码分析"
tags:
...
sources:
  - repo: nginx-gateway-fabric
    ...
    dirty: false
source_file: examples/externalname-service/route.yaml
source_sha256_12: da0d1e393806
runtime_snapshots: []
---

# examples/externalname-service/route.yaml 源码分析
...
|---:|---|---|---|---|
| 1 | `gateway.networking.k8s.io/v1` | `HTTPRoute` | `default/继承 kubectl 上下文` | `httpbin-route` |

**源码事实**：文件指纹为 `da0d1e393806`，分析基于 revision `87e0580143fc`。本文没有把案例实际部署到集群，因此“生成了某条 NGINX 指令”属于源码可推导的预期，不是运行观察。
...
| 对象 | 字段 | 案例值 | 进入系统后的含义 |
|---:|---|---|---|
| 1 | `metadata.name` | `httpbin-route` | 组成 NamespacedName，是 Graph、引用解析与生成文件命名的稳定身份。 |
| 1 | `spec.parentRefs[0].name` | `external-gateway` | Route→listener 附着入口；group/kind/name/sectionName/namespace 决定候选父对象。 |
...
| 1 | `spec.rules[0].filters[0].type` | `URLRewrite` | 按类型进入 header、CORS、redirect/rewrite、mirror 或 ExtensionRef 处理。 |
| 1 | `spec.rules[0].filters[0].urlRewrite` | `{...}` | 按类型进入 header、CORS、redirect/rewrite、mirror 或 ExtensionRef 处理。 |
| 1 | `spec.rules[0].backendRefs[0].name` | `httpbin` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
| 1 | `spec.rules[0].backendRefs[0].port` | `80` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
| 1 | `spec.rules[1].matches[0].path` | `{...}` | 转为 path/method/header/query 或 gRPC service/method 匹配条件。 |
| 1 | `spec.rules[1].backendRefs[0].name` | `coffee` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
| 1 | `spec.rules[1].backendRefs[0].port` | `80` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
...
1. 静态校验：`kubectl apply --dry-run=server -f examples/externalname-service/route.yaml`。
...
- [[externalname-service/gateway-yaml|gateway.yaml]]
- [[00-首页-学习路线]]
- [[99-源码索引与术语表]]
