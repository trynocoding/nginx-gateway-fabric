---
title: "examples/cross-namespace-routing/cafe-routes.yaml 源码分析"
tags:
...
sources:
  - repo: nginx-gateway-fabric
    ...
    dirty: false
source_file: examples/cross-namespace-routing/cafe-routes.yaml
source_sha256_12: 3572dc8198a3
runtime_snapshots: []
---

# examples/cross-namespace-routing/cafe-routes.yaml 源码分析
...
**源码事实**：文件指纹为 `3572dc8198a3`，分析基于 revision `87e0580143fc`。本文没有把案例实际部署到集群，因此“生成了某条 NGINX 指令”属于源码可推导的预期，不是运行观察。
...
| 1 | `spec.rules[0].backendRefs[0].namespace` | `cafe` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
...
| 2 | `spec.rules[0].backendRefs[0].namespace` | `cafe` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
...
1. 静态校验：`kubectl apply --dry-run=server -f examples/cross-namespace-routing/cafe-routes.yaml`。
...
- [[cross-namespace-routing/reference-grant-yaml|reference-grant.yaml]]
- [[00-首页-学习路线]]
- [[99-源码索引与术语表]]
