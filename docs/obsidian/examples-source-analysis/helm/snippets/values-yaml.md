---
title: "examples/helm/snippets/values.yaml Helm values 源码分析"
...
sources:
  - repo: nginx-gateway-fabric
    ...
    dirty: false
source_file: examples/helm/snippets/values.yaml
source_sha256_12: 2fb2bf13d2a3
---

# examples/helm/snippets/values.yaml Helm values 源码分析
...
| `nginxGateway.snippets.enable` | `true` | `templates/deployment.yaml` 生成 `--snippets`，`_helpers.tpl` 增加 SnippetsPolicy RBAC |
...
```bash
helm template ngf charts/nginx-gateway-fabric -f examples/helm/snippets/values.yaml
helm lint charts/nginx-gateway-fabric -f examples/helm/snippets/values.yaml
```
