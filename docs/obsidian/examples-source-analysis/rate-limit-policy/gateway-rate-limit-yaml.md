---
title: "examples/rate-limit-policy/gateway-rate-limit.yaml 源码分析"
tags:
...
sources:
  - repo: nginx-gateway-fabric
    ...
    dirty: false
source_file: examples/rate-limit-policy/gateway-rate-limit.yaml
source_sha256_12: 395997beaf62
runtime_snapshots: []
---

# examples/rate-limit-policy/gateway-rate-limit.yaml 源码分析

> [!abstract] 核心结论
> 该文件声明 1 个对象（RateLimitPolicy）。它的核心价值不是展示 YAML 语法，而是提供一段可追踪输入：受管对象经过 Graph 验证与引用解析，普通工作负载则只通过 Service/Secret 等边界间接影响 NGF。

## 为什么选它

此文件属于二八抽样中的代表输入：它覆盖 **RateLimitPolicy** 的关键处理面，并能与同目录文件组成完整或近完整的因果链。重复的 Deployment/Service 变体、只改变名称的 Route 和操作型 README 不再逐篇展开。

## 文件现场与职责边界

| 文档 | API 版本 | Kind | Namespace | Name |
|---:|---|---|---|---|
| 1 | `gateway.nginx.org/v1alpha1` | `RateLimitPolicy` | `default/继承 kubectl 上下文` | `gateway-rate-limit` |

**源码事实**：文件指纹为 `395997beaf62`，分析基于 revision `87e0580143fc`。本文没有把案例实际部署到集群，因此“生成了某条 NGINX 指令”属于源码可推导的预期，不是运行观察。
...
| 对象 | 字段 | 案例值 | 进入系统后的含义 |
|---:|---|---|---|
| 1 | `metadata.name` | `gateway-rate-limit` | 组成 NamespacedName，是 Graph、引用解析与生成文件命名的稳定身份。 |
| 1 | `spec.targetRefs[0].group` | `gateway.networking.k8s.io` | Policy attachment 入口；目标类型与作用域决定策略进入 server、location 或 upstream。 |
| 1 | `spec.targetRefs[0].kind` | `Gateway` | Policy attachment 入口；目标类型与作用域决定策略进入 server、location 或 upstream。 |
| 1 | `spec.targetRefs[0].name` | `gateway` | Policy attachment 入口；目标类型与作用域决定策略进入 server、location 或 upstream。 |
| 1 | `spec.rateLimit.local.rules[0].zoneSize` | `10m` | 转为共享内存 zone 与 limit_req 参数，默认值/文件命名由 ratelimit generator 负责。 |
| 1 | `spec.rateLimit.local.rules[0].key` | `$binary_remote_addr` | 转为共享内存 zone 与 limit_req 参数，默认值/文件命名由 ratelimit generator 负责。 |
| 1 | `spec.rateLimit.local.rules[0].rate` | `10r/s` | 转为共享内存 zone 与 limit_req 参数，默认值/文件命名由 ratelimit generator 负责。 |
...
|---|---|---|---|
| `RateLimitPolicy` | `internal/controller/state/graph/policies.go:processPolicies` | Route 级策略同时产生 http 级 limit_req_zone 影子配置和 location 级 limit_req；Gateway 级落到 server | `internal/controller/nginx/config/policies/ratelimit/validator_test.go` 与 `generator_test.go` |
...
1. 静态校验：`kubectl apply --dry-run=server -f examples/rate-limit-policy/gateway-rate-limit.yaml`。
...
4. 回归测试入口：`internal/controller/nginx/config/policies/ratelimit/validator_test.go` 与 `generator_test.go`。
