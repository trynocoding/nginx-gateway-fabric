---
title: "examples/advanced-routing/regex-route.yaml 源码分析"
tags:
...
|---:|---|---|---|---|
| 1 | `gateway.networking.k8s.io/v1` | `HTTPRoute` | `default/继承 kubectl 上下文` | `coffee-regex` |

**源码事实**：文件指纹为 `3864fab33edd`，分析基于 revision `87e0580143fc`。本文没有把案例实际部署到集群，因此“生成了某条 NGINX 指令”属于源码可推导的预期，不是运行观察。
...
| 对象 | 字段 | 案例值 | 进入系统后的含义 |
|---:|---|---|---|
| 1 | `metadata.name` | `coffee-regex` | 组成 NamespacedName，是 Graph、引用解析与生成文件命名的稳定身份。 |
| 1 | `spec.parentRefs[0].name` | `cafe` | Route→listener 附着入口；group/kind/name/sectionName/namespace 决定候选父对象。 |
...
| 1 | `spec.rules[0].backendRefs[0].name` | `coffee-v1-svc` | 规则→backend 的数据谱系；name/port/weight 决定 upstream 身份、端口与流量比例。 |
...
1. 静态校验：`kubectl apply --dry-run=server -f examples/advanced-routing/regex-route.yaml`。
